      ...
      <div class="atras col-xs-12 col-sm-7 col-md-8">
        <div class="enfoque2 col-xs-12 col-sm-12 col-md-12">
          <div class="enfoque col-xs-12 col-sm-12 col-md-12">
            ...
            <br>
            <form class="" action="crearP.php" method="post">
        ...
        <?php
        if ($_SESSION["mostrarP"]=="SI") {
          echo "<div class='enfoque2 col-xs-12 col-sm-12 col-md-12'>
            <div class='enfoque3 col-xs-12 col-sm-12 col-md-12'>";
          $anio=$_SESSION["anio"];
          echo "<h2>Lista de Periodos Creados:</h2><br>
          <div class='table-responsive'>
            <table class='table table-bordered table-hover table-condensed'><table-striped>";
              $consulta="select id_periodo from periodo where anio_id_anio='$anio'";//BASE DE DATOS
              $resultado= mysqli_query($conexion,$consulta);//BASE DE DATOS
              $fila=mysqli_fetch_all($resultado);//BASE DE DATOS
              $tamaño = sizeof($fila);
              if ($tamaño>0) {
                ...
                echo "<tr>
                  <th class='warning'>No se han encontrado Periodos registrados</th>
                </tr>";
              }
          echo "</table>
          </div> </div>
          ...
          <div class='enfoque col-xs-12 col-sm-12 col-md-12'>
            <br>
            <form class='' action='crearPP.php' method='post'>
              <h4>Mes: </h4><select class='form-control' name='periodo'>
             ...
             if ($_SESSION["crearP"]=="SI") {
                $_SESSION["crearP"]="NO";
                $_SESSION["mostrarP"]="NO";
                 $periodo = $_SESSION["periodo"];
                  $consulta="INSERT INTO `periodo` (`id_periodo`, `anio_id_anio`) VALUES ('$periodo', '$anio');";//BASE DE DATOS
                  $resultado= mysqli_query($conexion,$consulta);//BASE DE DATOS
                  if ($resultado != 0) {
                    echo '<script language="javascript">alert("Se ha creado el Periodo");</script>';
                  }else {
                    echo '<script language="javascript">alert("No Ha sido posible crear el Periodo");</script>';
                  }
             }elseif ($_SESSION["eliminarP"]=="SI") {
               $_SESSION["eliminarP"]="NO";
               $_SESSION["mostrarP"]="NO";
               $periodo = $_SESSION["periodo"];
                $consulta="select id_periodo from periodo where id_periodo= '$periodo' and anio_id_anio='$anio';";//BASE DE DATOS
                $resultado= mysqli_query($conexion,$consulta);//BASE DE DATOS
                $fila=mysqli_fetch_row($resultado);//BASE DE DATOS
                $tamaño = sizeof($fila);
                if ($tamaño == 1) {
                  echo '<script language="javascript">alert("Se ha eliminado el Perido");</script>';
                }else {
                  echo '<script language="javascript">alert("No Ha sido posible eliminar el Periodo");</script>';
                }
                $consulta="DELETE FROM `periodo` WHERE `periodo`.`id_periodo` = '$periodo' AND `periodo`.`anio_id_anio` = '$anio';";//BASE DE DATOS
                $resultado= mysqli_query($conexion,$consulta);//BASE DE DATOS
             }
          echo "</div></div>";
        }
